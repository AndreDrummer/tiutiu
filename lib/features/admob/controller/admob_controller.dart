...
class AdMobController {
      ...
      try {
        await RewardedAd.load(
          adUnitId: systemController.getAdMobBlockID(
              blockName: adBlockName, type: AdMobType.rewarded),
          request: AdRequest(),
    ...
    if (Platform.isAndroid || Platform.isIOS) {
      if (!_rewardedAdWasLoaded) {
        contactType == ContactType.whatsapp
            ? await loadWhatsAppRewardedAd()
            : await loadChatRewardedAd();
      }
      ...
      if (allowGoogleAds) {
        _rewardedAd.show(
          onUserEarnedReward: (AdWithoutView ad, RewardItem rewardItem) async {
            ...
            if (kDebugMode)
              debugPrint(
                  'TiuTiuApp: Usuário assistiu certinho ${ad.adUnitId} ${rewardItem.amount}');
  ...
  Future<void> loadOpeningAd() async {
    if (Platform.isAndroid || Platform.isIOS) {
      final String androidIntertitialAdId =
          'ca-app-pub-2837828701670824/9465656437';
      final String iOSIntertitialAdId =
          'ca-app-pub-2837828701670824/3404285743';
      final String adTestId = 'ca-app-pub-3940256099942544/4411468910';
      final bool isIOS = Platform.isIOS;

      final String prodAdId =
          isIOS ? iOSIntertitialAdId : androidIntertitialAdId;
  ...
  Future<void> showOpeningAd() async {
    if (Platform.isAndroid || Platform.isIOS) {
      final allowAdOnOpening =
          adminRemoteConfigController.configs.allowAdOnOpening;
      final allowGoogleAds = adminRemoteConfigController.configs.allowGoogleAds;

      if (!_interstitialAdWasLoaded) {
        debugPrint('Intertitial Ad NOT loaded.');
        await loadOpeningAd();
      } else if (allowAdOnOpening &&
          !_interstitialAdWasShown &&
          allowGoogleAds) {
